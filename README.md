...
- **歌单与详情**：查看歌曲详情、专辑信息及播放列表，支持添加快捷方式、一键解析播放。
...
- **批量下载**：支持自定义下载功能，可批量下载歌曲。
- **本地音频支持**：支持自动扫描本地音频文件、手动添加本地文件并直接播放。
- **歌曲收藏**：支持在应用内收藏歌曲，本地音频和网易云歌曲均可加入收藏。
- **听力保护**：提供智能听力保护，开启后连续听歌达到设定时长会自动暂停提醒休息。

### 🗂️ 数据与设置

- **数据导入导出**：支持将设置、主页快捷方式等使用数据加密导出与导入，便于备份和迁移。导出按钮位于设置界面右上角。
...
- **Android 版本**：`Android 12.0 / API Level 26.0` 及以上。
- **必要权限**：
  - 悬浮歌词需“显示在其他应用上层”权限。
...
2.  **播放**: 点击歌曲即可开始播放。
3.  **本地音频**: 可自动扫描设备中的本地音频文件，也支持手动添加文件后播放，还可以直接从文件管理器选择本应用打开音频文件。
4.  **收藏与下载**: 可在应用内收藏本地或网易云歌曲，也可使用自定义下载功能批量下载歌曲。
5.  **悬浮歌词**: 在“设置”中启用“悬浮歌词”功能。开启后将出现一个悬浮图标/窗口，点击即可展开查看控制选项和设置。
6.  **设置与数据迁移**: 可配置音频质量、智能听力保护，并输入您的网易 `MUSIC_U` cookie 进行认证访问；同时支持加密导出、导入设置和主页快捷方式等数据。（备份数据加密，文件格式为 `.mlns` ）
    > 关于 `MUSIC_U` 的获取，请参考 [获取 `MUSIC_U` 的方法](https://github.com/midairlogn/ML-Netease_url/blob/main/MUSIC_U/get-MUSIC_U.md).

## ⚠️ 免责声明

本工具仅供学习交流使用，非网易云音乐官方应用。请支持正版音乐。使用本工具时请遵守相关法律法规，尊重音乐人的劳动成果。
